...
function Home() {
  ...
  useEffect(() => {
   fetch('https://mymoviz-backend-pearl-six.vercel.app/movies')
     ...
     .then(data => {
      const moviesArray = data.movies.map(film => ({
        ...
        overview: film.overview.length > 250 ? film.overview.slice(0,250) + "..." : film.overview
      }));

      setDataMovies(moviesArray);
     });
 }, []);
  ...
  const movies = dataMovies.map((data, i) => {
